...
void handle_client(int client_socket, list_t **todo_list, int *id_count)
{
	...
	else if (!strncmp(msg, "GET /todos?id=", 14))
		error = get_todo(client_socket, *todo_list, get_id(msg, 14));
	else if (!strncmp(msg, "DELETE /todos?id=", 17))
		error = delete_todo(client_socket, todo_list, get_id(msg, 17));
	else
		error = 404;
	if (error)
